...
import { ArrowLeft, MapPin, Save, CheckCircle2, ChevronDown, Calendar, Hash, Layers, ChevronRight } from "lucide-react";
...
export default function VisitRegistrationPage({ params }: { params: { id: string } }) {
  ...
  const canProceedStep1 = selectedProject !== "";
  const canProceedStep2 = lote.trim() !== "" && etapa.trim() !== "";
  const canSubmit = visitDate !== "" && visitTime !== "";
    ...
    <div className="font-display bg-[#f6f8f8] text-slate-900 min-h-screen relative">
      <div className="relative flex h-auto min-h-screen w-full flex-col overflow-x-hidden max-w-lg mx-auto bg-white shadow-xl">
        <header className="flex items-center p-4 border-b border-primary/10 justify-between sticky top-0 bg-white/80 backdrop-blur-md z-10">
          ...
          <h2 className="text-slate-900 text-lg font-black leading-tight tracking-tight flex-1 text-center uppercase">Programar Visita</h2>
        ...
        <div className="px-6 pt-4 pb-2">
          ...
          <div className="flex justify-between mt-2">
            <span className={clsx("text-[9px] font-black uppercase tracking-widest", currentStep >= 1 ? "text-primary" : "text-slate-300")}>Proyecto</span>
            <span className={clsx("text-[9px] font-black uppercase tracking-widest", currentStep >= 2 ? "text-primary" : "text-slate-300")}>Terreno</span>
            <span className={clsx("text-[9px] font-black uppercase tracking-widest", currentStep >= 3 ? "text-primary" : "text-slate-300")}>Fecha</span>
          </div>
        </div>
          ...
          {currentStep === 1 && (
            <section className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="text-center mb-6">
                ...
                <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">Seleccionar Proyecto</h3>
                <p className="text-slate-400 text-xs font-bold mt-1">¿A qué proyecto desea llevar al cliente?</p>
              </div>
          ...
          {currentStep === 2 && (
            <section className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="text-center mb-6">
                ...
                <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">Datos del Terreno</h3>
                <p className="text-slate-400 text-xs font-bold mt-1">
                  {PROJECTS.find(p => p.id === selectedProject)?.emoji} {PROJECTS.find(p => p.id === selectedProject)?.name}
                </p>
              </div>
              ...
              {/* Etapa */}
              <div className="space-y-2">
                <label className="text-slate-400 text-[10px] font-black uppercase tracking-widest px-1 block">Etapa del Terreno</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Ej: Etapa 1, Etapa 2, etc."
                    className="w-full pl-12 pr-4 py-5 bg-white border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all text-slate-700 font-bold text-lg outline-none"
                  />
                </div>
              </div>
          ...
          {currentStep === 3 && (
            <section className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="text-center mb-6">
                ...
                <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">Coordinar Fecha</h3>
                <p className="text-slate-400 text-xs font-bold mt-1">
                  {PROJECTS.find(p => p.id === selectedProject)?.emoji} {PROJECTS.find(p => p.id === selectedProject)?.name} — Lote {lote}, {etapa}
                </p>
              </div>

              {/* Date Picker */}
              <div className="space-y-2">
                <label className="text-slate-400 text-[10px] font-black uppercase tracking-widest px-1 block">Fecha de Visita</label>
                <div className="relative">
              ...
              {/* Time Picker */}
              <div className="space-y-2">
                <label className="text-slate-400 text-[10px] font-black uppercase tracking-widest px-1 block">Hora de Visita</label>
                <div className="relative">
                  <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 text-primary" size={20} />
                  <input
                    type="time"
              ...
              {/* Summary Card */}
              {visitDate && visitTime && (
                <div className="bg-primary/5 border border-primary/10 rounded-2xl p-5 space-y-2 animate-in fade-in duration-300">
                  <p className="text-[10px] font-black uppercase tracking-widest text-primary">Resumen de la Visita</p>
                  ...
                  <p className="text-slate-700 font-bold text-sm">
                    📅 {new Date(visitDate + "T" + visitTime).toLocaleString("es-CL", { 
              ...
              <p className="text-center text-slate-400 text-[10px] pt-2 uppercase tracking-[0.2em] font-black">
      ...
      {showSuccess && (
        <div className="fixed inset-x-0 bottom-10 px-4 flex justify-center z-[100] animate-bounce">
          <div className="bg-slate-900 shadow-2xl text-white px-8 py-4 rounded-full flex items-center gap-3 border border-white/10">
            ...
            <span className="font-black uppercase text-sm tracking-widest">¡Visita programada con éxito!</span>
          </div>
        </div>
      )}
    </div>
